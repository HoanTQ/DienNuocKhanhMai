...
description: >
  ...
  LUÔN kích hoạt skill này khi user đưa vào tài liệu spec, SRS, PRD, Use Case có sẵn — dù user
  chỉ nói "làm theo tài liệu này", "implement theo spec này", hoặc "code theo UC này".
  Skill này ưu tiên format và cấu trúc Use Case của nó hơn format gốc của tài liệu đầu vào.
...
## ⚡ QUY TẮC ƯU TIÊN (LUÔN ÁP DỤNG)

> **CRITICAL:** Skill này PHẢI được kích hoạt (activate) trong MỌI trường hợp liên quan đến spec/SRS/Use Case,
> kể cả khi user đưa vào tài liệu có sẵn và chỉ yêu cầu "làm theo" hoặc "implement theo".

### Khi user đưa tài liệu spec/SRS/PRD/Use Case có sẵn vào:

1. **TỰ ĐỘNG kích hoạt skill này** — không cần user yêu cầu activate
2. **ƯU TIÊN format và template Use Case của skill này** hơn format gốc của tài liệu đầu vào
3. **Re-structure nội dung** theo template Use Case ở Phase 2 (STRUCTURE) bên dưới
4. **Bổ sung các phần còn thiếu** so với template chuẩn (đánh dấu `[TBD]` nếu không có info)
5. **Chạy VERIFY (Phase 3)** trên nội dung đã re-structure — phát hiện gaps và mâu thuẫn
6. **Giữ nguyên nội dung nghiệp vụ** — chỉ thay đổi cấu trúc và format

### Trigger conditions (kích hoạt ngay khi gặp BẤT KỲ điều nào):

- User paste/đính kèm tài liệu spec, SRS, PRD, Use Case, user story
- User nói: "làm theo tài liệu này", "implement theo spec", "code theo UC này"
- User đề cập: "viết spec", "phân tích yêu cầu", "đặc tả", "use case", "requirement"
- User mô tả feature mới hoặc yêu cầu cấu trúc hóa ý tưởng
- User nói: "mình cần tính năng...", "feature này làm...", "spec cho..."

### Workflow khi nhận tài liệu có sẵn:
INPUT: Tài liệu spec/SRS/PRD/Use Case có sẵn từ user 
↓
STEP 1: Đọc và phân tích nội dung tài liệu gốc (path READ_EXISTING trong bảng quyết định) 
↓
STEP 2: Map nội dung vào template Use Case chuẩn của skill này
↓
STEP 3: Chạy GAP_ANALYSIS — identify những gì template yêu cầu nhưng tài liệu gốc thiếu
↓
STEP 4: Hỏi user về các gaps critical hoặc đánh dấu [TBD]
↓
STEP 5: Chạy VERIFY (consistency + completeness checks)
↓
STEP 6: Xuất Use Case document + Mermaid diagrams + Task breakdown theo format chuẩn


### Ưu tiên khi có conflict giữa tài liệu gốc và skill:

| Aspect | Ưu tiên | Lý do |
|--------|---------|-------|
| **Nội dung nghiệp vụ** | Tài liệu gốc | Đây là source of truth về business |
| **Format / cấu trúc** | Skill này | Template Use Case 10 sections chuẩn |
| **ID convention** | Skill này | UC-XX, BR-XX, EX-XX, AF-XX |
| **Diagram** | Skill này | Luôn dùng Mermaid (sequence + state) |
| **Verify logic** | Skill này | Luôn chạy Phase 3 VERIFY |
| **Ngôn ngữ viết** | Theo user | Tiếng Việt hoặc Anh tùy context |
| **Sections bắt buộc** | Skill này | Tất cả 10 sections trong template |
