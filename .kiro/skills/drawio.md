...
## Swimlane Process Diagrams (Cross-lane flows)

When drawing swimlane process diagrams (e.g. business process flows with multiple roles):

### Structure rules
- Each lane is an **independent** `swimlane` cell with `parent="1"` — do NOT nest lanes inside a pool container with `childLayout=stackLayout` (this breaks coordinate control)
- Lane label: `horizontal=0` makes the label vertical (rotated), which is standard for horizontal swimlanes
- Lanes are stacked vertically by setting sequential `y` coordinates: lane1.y, lane2.y = lane1.y + lane1.height, etc.
- Add a `title` text cell above all lanes for the diagram title

### Element placement
- Elements **within a lane** use `parent="laneId"` with coordinates **relative to the lane** (not the page)
- Leave at least **30px margin** from lane top/bottom edges so elements don't touch lane borders
- Space elements horizontally with at least **50px gap** between them
- Align elements in the same logical row to the **same y coordinate** across lanes

### Edge (connector) rules
- Edges **within the same lane**: use `parent="laneId"`
- Edges **crossing lanes**: use `parent="1"` (root layer) — this is critical, otherwise edges render incorrectly
- Always use `edgeStyle=orthogonalEdgeStyle;rounded=0;` for clean right-angle connectors

### Lane sizing
- Make lanes **tall enough** for their content + margins. Minimum 160px, typical 200-220px
- If a lane has elements in 2 rows (e.g. a branch going down), increase height to 280-320px
- End events should be placed in the lane of the actor who completes the process

### Example lane structure
```xml
<mxCell id="lane1" value="Role A" style="swimlane;startSize=120;horizontal=0;fillColor=#dae8fc;strokeColor=#6c8ebf;fontStyle=1;fontSize=12;" vertex="1" parent="1">
  <mxGeometry x="30" y="60" width="1580" height="200" as="geometry" />
</mxCell>
<mxCell id="lane2" value="Role B" style="swimlane;startSize=120;horizontal=0;fillColor=#d5e8d4;strokeColor=#82b366;fontStyle=1;fontSize=12;" vertex="1" parent="1">
  <mxGeometry x="30" y="260" width="1580" height="160" as="geometry" />
</mxCell>
<mxCell id="nodeA" value="Task A" style="rounded=1;whiteSpace=wrap;html=1;" vertex="1" parent="lane1">
  <mxGeometry x="200" y="65" width="150" height="70" as="geometry" />
</mxCell>
<mxCell id="nodeB" value="Task B" style="rounded=1;whiteSpace=wrap;html=1;" vertex="1" parent="lane2">
  <mxGeometry x="200" y="45" width="150" height="70" as="geometry" />
</mxCell>
<mxCell id="cross_edge" style="edgeStyle=orthogonalEdgeStyle;rounded=0;" edge="1" source="nodeA" target="nodeB" parent="1">
  <mxGeometry relative="1" as="geometry" />
</mxCell>
```
