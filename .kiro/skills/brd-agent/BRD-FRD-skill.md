...
description: >
  ...
  SKILL NÀY PHẢI LUÔN ĐƯỢC KÍCH HOẠT (activate) khi conversation liên quan đến BRD, FRD,
  requirement, spec, tài liệu nghiệp vụ, tài liệu thiết kế, blueprint — kể cả khi user đưa vào tài liệu có sẵn.
---
...
## ⚡ QUY TẮC ƯU TIÊN TUYỆT ĐỐI (PRIORITY OVERRIDE RULES)

> **Các quy tắc này có mức ưu tiên cao nhất và PHẢI được tuân thủ trong mọi tình huống.**

### 1. Luôn tự động kích hoạt (Auto-Activate)

Skill này PHẢI được kích hoạt (activate) ngay lập tức khi phát hiện BẤT KỲ dấu hiệu nào sau:
- User đề cập: "BRD", "FRD", "spec", "requirement", "yêu cầu", "đặc tả", "tính năng", "feature", "luồng nghiệp vụ", "use case", "user story"
- User đưa vào (paste/attach/copy paste) một tài liệu BRD hoặc FRD có sẵn
- User yêu cầu: "viết theo tài liệu này", "làm theo BRD này", "implement theo spec","dựa trên tài liệu này"
- User đề cập đến stakeholder, business rule, acceptance criteria
- Bất kỳ context nào liên quan đến phân tích nghiệp vụ hoặc tài liệu kỹ thuật

### 2. Ưu tiên format của Skill này (Format Priority)

Khi user đưa vào tài liệu BRD/FRD có sẵn (từ bên ngoài), skill này PHẢI:

1. **Đọc và phân tích** tài liệu đầu vào
2. **Áp dụng format chuẩn** của skill này (template BRD/FRD bên dưới) lên nội dung
3. **Bổ sung** các section còn thiếu theo template (đánh dấu `[TBD]` nếu chưa có thông tin)
4. **Chuẩn hóa ID format**: `BR-XX`, `FR-XX`, `UC-XX`
5. **Thêm Mermaid diagrams** nếu tài liệu gốc chưa có
6. **Giữ nguyên thông tin gốc** — không được bỏ mất data, chỉ restructure và bổ sung

### 3. Khi user đưa tài liệu BRD/FRD có sẵn vào:

1. **Tự động kích hoạt skill này** — không cần user yêu cầu activate
2. **Ưu tiên format/template của skill này** hơn format gốc của tài liệu đầu vào
3. **Áp dụng workflow của skill** để:
   - Re-structure nội dung theo template BRD/FRD chuẩn ở Phase 4
   - Bổ sung các section còn thiếu (so với template)
   - Chuẩn hóa ID format (`BR-XX`, `FR-XX`, `UC-XX`)
   - Thêm Mermaid diagrams nếu tài liệu gốc chưa có
   - Đánh dấu `[TBD]` cho thông tin còn thiếu
4. **Hỏi fen để clarify** những điểm mơ hồ trong tài liệu gốc (Phase 2 rút gọn)
5. **Validate lại** trước khi xuất document cuối cùng (Phase 3)

### 4. Trigger tự động — Skill được activate khi phát hiện BẤT KỲ dấu hiệu nào sau:

- User paste/đính kèm nội dung có cấu trúc BRD/FRD/spec
- User nói: "làm theo tài liệu này", "implement cái này", "chuyển thành spec"
- User đưa file có chứa: requirements, use cases, user stories, acceptance criteria
- User nhắc đến: "tài liệu", "document", "spec", "yêu cầu", "đặc tả"
- User yêu cầu review/cải thiện tài liệu nghiệp vụ hiện có


### 5. Ưu tiên khi có conflict giữa tài liệu gốc và skill:

| Aspect | Ưu tiên | Lý do |
|--------|---------|-------|
| **Nội dung nghiệp vụ** | Tài liệu gốc | Đây là source of truth về business |
| **Format / cấu trúc** | Skill này | Đảm bảo consistency và completeness |
| **ID convention** | Skill này | BR-XX, FR-XX, UC-XX |
| **Diagram** | Skill này | Luôn dùng Mermaid |
| **Ngôn ngữ viết** | Theo user | Tiếng Việt hoặc Anh tùy context |
| **Sections bắt buộc** | Skill này | Scope, Stakeholders, Risks, AC |


> ⚠️ **KHÔNG BAO GIỜ** chỉ copy-paste tài liệu gốc mà không áp dụng template của skill.
...
**Ngoại lệ:** Khi user đưa tài liệu có sẵn vào, chuyển sang workflow "Existing Document Handling" (xem mục 3 ở trên) thay vì bắt đầu từ Phase 1.
---
